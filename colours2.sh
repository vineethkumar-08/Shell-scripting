...
echo  -e "$R Please run this script with root acess $N" | tee -a $LOGS_FILE     #run as root user 
...
VALIDATE(){
    ...
    if [ $1 -ne 0 ] ; then
        echo -e "$R $2 installation failed... $N" | tee -a $LOGS_FILE # error message 
        exit 1
    else
        echo -e "$2 $B Instalation sucessful.... $Y"  | tee -a $LOGS_FILE
fi
